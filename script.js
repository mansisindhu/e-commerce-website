...
function renderCheckout() {
  ...
  couponBtnElement.addEventListener("click", function () {
    ...
    if (inputBoxElement.value === "" || inputBoxElement.value !== "masai30") {
      ...
      priceElement.textContent = `Total cost after discount - Rs ${Math.ceil(
        window.history.state.data.totalPrice * 0.7
      )}`;
      priceElement.style.color = "teal";

      alert(
        `You saved Rs ${
          window.history.state.data.totalPrice -
          Math.ceil(window.history.state.data.totalPrice * 0.7)
        } `
      );
    }
  });
  ...
  checkoutBtnElement.addEventListener("click", function () {
    ...
    checkoutImage.src =
      "https://images.pexels.com/photos/2072165/pexels-photo-2072165.jpeg?auto=compress&cs=tinysrgb&dpr=1&w=500";
...
function onAdd() {
  ...
  const exists = appData["local-store-products"].filter(
    (el) => el.id === Number(idElement.value)
  );
  console.log(exists);
...
function renderApp() {
  ...
  appData["local-store-products"] =
    getFromLocalStorage("local-store-products") || [];
